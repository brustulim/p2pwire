'use strict'
import { objectHasValue } from './util'
import { messageType } from './constants'

function sendMessage (peerConn, message) {
  validateMessage(message)
  console.log('MESSAGE -> sendMessage -> peerConn', peerConn)
  const msg = { t: messageType.CONSUMER_MESSAGE, d: peerConn.destinationNodeAddress, m: message }
...
function processReceivedMessage (event) {
  ...
  if (!message.t || !objectHasValue(messageType, message.t)) {
    throw new Error(
      'Invalid message type received from remote peer: ' + JSON.stringify(message, null, 2)
    )
  }
  return message
}
...
export default { sendMessage, sendConnectionsList, processReceivedMessage }
