...
class Discovery extends EventEmitter {
  ...
  createConnection (peer) {
    ...
    peerConn.on('signal', (data) => {
      ...
      peer.send(JSON.stringify({ t: 'pcSignal', a: this.nodeAddress, data }))
    })
    ...
    peer.on('data', (data) => {
      try {
        ...
        if (typeof objData === 'object' && objData.t === 'pcSignal') {
          console.log(
          ...
          if (this.conn.isConnectedTo(peerConn.remoteNodeAddress)) {
            console.log(
              'DISCOVERY - createConnection - ON Peer data - REJECTING - already connected to: ', peerConn.remoteNodeAddress)
            peerConn.destroy(
              new Error(
